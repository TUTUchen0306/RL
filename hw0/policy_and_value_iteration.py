...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
        ...
        for s in range(num_spaces):
            ...
            for a in range(num_actions):
                for next_state in range(num_spaces):
                    v[a] += (P[s][a][next_state] * (R[s][a][next_state] + gamma * V[next_state]))
            
            max_v = max(v)
            dif = max(dif, np.abs(V[s]-max_v))
            V[s] = max_v
    ...
    for s in range(num_spaces):
        ...
        for a in range(num_actions):
            for next_state in range(num_spaces):
                q[a] += (P[s][a][next_state] * (R[s][a][next_state] + gamma * V[next_state]))
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for times in range(max_iterations):
        ...
        for s in range(num_spaces):
            ...
            a = policy[s]
            for next_state in range(num_spaces):
                v += (P[s][a][next_state] * (R[s][a][next_state] + gamma * V[next_state]))
            dif = max(dif, np.abs(V[s]-v))
            V[s] = v
